...
public:
  ...
  PatchWorkpp(ros::NodeHandle* nh) : node_handle_(*nh)
  {
    ...
    cout << (boost::format("Num. sectors: %d, %d, %d, %d") % num_sectors_each_zone_[0] %
             num_sectors_each_zone_[1] % num_sectors_each_zone_[2] %
             num_sectors_each_zone_[3])
                .str()
         << endl;
    cout << (boost::format("Num. rings: %01d, %01d, %01d, %01d") %
             num_rings_each_zone_[0] % num_rings_each_zone_[1] % num_rings_each_zone_[2] %
    ...
    PlaneViz =
        node_handle_.advertise<jsk_recognition_msgs::PolygonArray>("plane", 100, true);
    pub_revert_pc =
        node_handle_.advertise<sensor_msgs::PointCloud2>("revert_pc", 100, true);
    pub_reject_pc =
        node_handle_.advertise<sensor_msgs::PointCloud2>("reject_pc", 100, true);
    ...
    ring_sizes_ = {
        (min_range_z2_ - min_range_) / num_rings_each_zone_.at(0),
...
private:
  ...
  set_polygons(int zone_idx, int r_idx, int theta_idx, int num_split);

  void set_ground_likelihood_estimation_status(
      const int zone_idx, const int ring_idx, const int concentric_idx,
      const double z_vec, const double z_elevation, const double ground_flatness);
};
           ...
           ++sector_idx)
      {
        ...
        if (visualize_)
        {
          ...
          set_ground_likelihood_estimation_status(
              zone_idx, ring_idx, concentric_idx, ground_uprightness, ground_elevation,
              ground_flatness);
...
PatchWorkpp<PointT>::set_polygons(int zone_idx, int r_idx, int theta_idx, int num_split)
{
